...
class UserDocumentsBuilder extends StatelessWidget {
  final Function()? onSuccess;
  final Function()? onSubmitPressed;
  final Function(String? error)? onError;

  const UserDocumentsBuilder({
    Key? key,
    this.onSuccess,
  ...
  Widget build(BuildContext context) {
    return BlocConsumer(
      bloc: context.read<UserDocumentBloc>(),
      listener: (context, UiState state) {
        if (state is Success &&
            (context.read<UserDocumentBloc>().state.getData()?.success ??
                false)) {
          onSuccess?.call();
        } else if (!(context
            ...
            false)) {
          onError
              ?.call(context.read<UserDocumentBloc>().state.getData()?.message);
        }
        if (state is Error) {
          CommonErrorHandler(
            context,
