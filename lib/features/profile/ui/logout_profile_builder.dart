...
class LogoutProfileBuilder extends StatelessWidget {
  final Function()? onSetAsPrimaryPressed;

  const LogoutProfileBuilder({
    Key? key,
      ...
      listener: (context, state) {
        if (state is Success) {
          ...
          AppBottomSheet.showAppSnackBar(
            context: context,
            text: "Profile Logout Successfully",
          );
          Navigator.pushAndRemoveUntil(
            ...
            (!(context.read<LogoutBloc>().state.getData()?.success ?? false))) {
          AppActionDialog.showActionDialog(
            image: "$icons_path/ErrorIcon.png",
            context: context,
            title: "Error occurred",
            subtitle: context.read<LogoutBloc>().state.getData()?.message ??
