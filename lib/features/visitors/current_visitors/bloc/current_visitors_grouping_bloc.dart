...
import 'package:host_visitor_connect/features/visitors/current_visitors/data/repos/current_visitor_repository.dart';
...
class CurrentVisitorsGroupingBloc extends Cubit<UiState<List<Room>>> {
  PageResponse<VisitorRoomResponse>? _currentPageResponse;
  final CurrentVisitorRepository _visitorRepository;
  List<Room> rooms = [];
  late Completer refreshCompleter;
  int? orderByValue;

  CurrentVisitorsGroupingBloc({
    CurrentVisitorRepository? visitorRepository,
  })  : _visitorRepository = visitorRepository ?? CurrentVisitorRepository(),
        super(Default());

  Future currentVisitorsGrouping({
    int pageNo = 0,
