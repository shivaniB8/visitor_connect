  ...
  Widget build(BuildContext context) {
    return BlocConsumer(
      ...
      listener: (context, UiState state) {
        if (state is Success) {
          if (context.read<RequestOtpBloc>().state.getData()?.success ??
              false) {
            onSuccess?.call();
          } else {
            aadharerror?.call(
              context.read<RequestOtpBloc>().state.getData()?.message ??
                  'Invalid Aadhar Or Number',
            );
          }
        }
        if (state is Error) {
